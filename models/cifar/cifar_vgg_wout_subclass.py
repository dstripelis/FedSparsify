...
class VGG__(object):
	...
	def __init__(self, vgg_name, num_classes):
		...
		self._make_layers(self.config[vgg_name])
	...
	def _make_layers(self, config):
		...
		for l in config:
			if l == 'M':
				...
				self.model.add(layers.Conv2D(l, kernel_size=3, padding='same'))
				self.model.add(layers.BatchNormalization(trainable=False))
				self.model.add(layers.ReLU())
		self.model.add(layers.AveragePooling2D(pool_size=1, strides=1))
...
class CifarVGG(Model):
	...
	def get_model(self):
		self.model = VGG__(self.model_type, self.num_classes).model
